...
arr7 = get_data('../assets/silent_sorrow_7.png', '../assets/silent_sorrow_7.chr', 'SORROW_NAM_7_PART')
...
new_lines = []
for line in lines:
    if line.startswith('ct U[240] SORROW_NAM_7_PART'): continue
    new_lines.append(line)
content = '\n'.join(new_lines)

content = content.replace('file(fmt, "../assets/silent_sorrow_7.png")', 'file(raw, "../assets/silent_sorrow_7.chr")')

idx = content.find('fn sorrow_setup_nametable')
new_content = content[:idx] + arr7 + '\n\n' + content[idx:]

old_loop = '''    else if current_frame == 6
        {PPUSTATUS}()
        {PPUADDR}($20)
        {PPUADDR}($00)
        for U i = 0; i < 240; i += 1
...
new_loop = '''    else if current_frame == 6
        {PPUSTATUS}()
        {PPUADDR}($20)
        {PPUADDR}($00)
        for U i = 0; i < 240; i += 1
        ...
        {PPUADDR}($00)
        for U i = 0; i < 240; i += 1
            {PPUDATA}(SORROW_NAM_7_PART_0[i])
        for U i = 0; i < 240; i += 1
            {PPUDATA}(SORROW_NAM_7_PART_1[i])
        for U i = 0; i < 240; i += 1
            {PPUDATA}(SORROW_NAM_7_PART_2[i])
        for U i = 0; i < 240; i += 1
            {PPUDATA}(SORROW_NAM_7_PART_3[i])'''

if 'current_frame == 7' not in new_content:
