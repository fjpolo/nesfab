...
-- and tracks the true ground-truth game loop ticker 'fps_ticks' at $02F3.
...
function update_fps()
    ...
    local current_nmi_counter = memory.readbyte(0x000A)
    ...
    -- Read game's ground-truth game loop iteration counter 'fps_ticks' at $02F3
    local current_game_frame = memory.readbyte(0x02F3)
...
function draw_fps_graph()
    ...
    if emu.drawRectangle then
        -- Mesen API (semi-transparent black card)
        emu.drawRectangle(graph_x - 4, graph_y - 4, (graph_width * 2) + 8, graph_h + 8, 0xAA000000, true, 1)
        emu.drawString(graph_x + 4, graph_y + 4, string.format("FPS: %.1f", fps), 0xFFFFFF, 0x00000000)
    elseif gui.box then
        -- Fceux API
    ...
    if emu.drawLine then
        -- 60 FPS Guideline (Green)
        emu.drawLine(graph_x, y_60, graph_x + (graph_width * 2), y_60, 0x4000FF00, 1)
        emu.drawString(graph_x - 18, y_60 - 3, "60", 0x00FF00, 0x00000000)
        
        -- 30 FPS Guideline (Red)
        emu.drawLine(graph_x, y_30, graph_x + (graph_width * 2), y_30, 0x40FF0000, 1)
        emu.drawString(graph_x - 18, y_30 - 3, "30", 0xFF0000, 0x00000000)
    ...
    for i = 1, graph_width do
        ...
        local bar_x = graph_x + (i - 1) * 2
        
        -- Determine color based on performance tier
        local color = 0xFF00FF00 -- Green (Default)
        ...
        -- Draw single column bar
        if emu.drawRectangle then
            emu.drawRectangle(bar_x, bar_y, 2, bar_h, color, true, 1)
        elseif gui.box then
            gui.box(bar_x, bar_y, bar_x + 1, bottom_y, fceux_color)
        end
    end
end
